...
# with col1:
#     age = st.number_input( "Let us know your age",  placeholder="Enter your age...", label_visibility = "collapsed", min_value=0, max_value=123)
#     height = st.number_input( "Let us know your height", placeholder="Enter your height in cms...", label_visibility = "collapsed", min_value=0, max_value=272)
#     weight = st.number_input( "Let us know your weight", placeholder="Enter your weight in kgs...", label_visibility = "collapsed", min_value=0, max_value=635)
#     work = st.selectbox("Let us know your age", ("Never Worked", "Children", "Private","Goverment Job","Self Employed"),  placeholder="Select your work type", label_visibility = "collapsed")
#     smoking = st.selectbox("Let us know your smoking habit", ("Never Smoked", "Formerly Smoked", "Smokes", "Unknown"), placeholder="Select your smoking habit",label_visibility = "collapsed" )
#     glucose = st.number_input( "Let us know your sugar level", placeholder="Enter your average blood sugar", label_visibility = "collapsed", min_value=0, max_value=2656)
...
#     residence = st.radio("Select your residence type", ["Rural", "Urban"], horizontal=True)    
...
def change_format(age, height, weight, work, smoking, glucose, gender, married, residence, hypertension, heart_disease):
    ...
    input_values = [
        age, glucose, bmi, gender, hypertension, heart_disease,
        married, work, residence, smoking
    ]
    ...
    return pd.DataFrame([input_values], columns=column_names)

def reset():
    pass  # Reset logic not implemented yet

col1, col2 = st.columns(2)

with col1:
    age = st.number_input("Age", min_value=0, max_value=123)
    height = st.number_input("Height (cm)", min_value=0, max_value=272)
    weight = st.number_input("Weight (kg)", min_value=0, max_value=635)
    work = st.selectbox("Work Type", ("Never Worked", "Children", "Private","Goverment Job","Self Employed"))
    smoking = st.selectbox("Smoking Habit", ("Never Smoked", "Formerly Smoked", "Smokes", "Unknown"))
    glucose = st.number_input("Average Glucose Level", min_value=0, max_value=2656)

with col2:
    gender = st.radio("Gender", ["Male", "Female"], horizontal=True)
    married = st.checkbox("I am/was married")
    residence = st.radio("Residence Type", ["Rural", "Urban"], horizontal=True)
    hypertension = st.checkbox("I have Hypertension")
    heart_disease = st.checkbox("I have a Heart Disease")
...
if st.button("Predict"):
    user_data = change_format(
        age, height, weight, work, smoking, glucose,
        gender, married, residence, hypertension, heart_disease
    )
    pred = predictor_model.predict(user_data)
